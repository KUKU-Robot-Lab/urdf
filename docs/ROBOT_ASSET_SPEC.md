...
**★★벤더가 완성된 Isaac Sim 자산을 직접 주는 경우가 있다 — 그게 sim 물리값의 1순위 출처다.**
Tesollo 는 `repo/tesollo/tesollo_model/dg5f*/usd/*/configuration/*_physics.usd` 에 자기
Isaac Sim 자산을 배포한다. 여기서 읽는 값(2026-09-10 확인, dg5f / dg5f-short / dg5fs /
15dof 전 변종 동일):

| 항목 | 값 |
|---|---|
| `drive:angular:physics:maxForce` | 7.5 N·m |
| `physxJoint:maxJointVelocity` | 180 °/s (= π rad/s) |
...
| 드라이브 게인 | **관성 비례** — `stiffness = 625 × JointEquivalentInertia`, `damping = 0.25 × I` (USD 각도 단위) |
| joint friction · armature | **없음** (그러므로 넣지 않는다) |

게인은 **숫자를 베끼지 말고 규칙을 우리 자산의 `physics:JointEquivalentInertia` 에 적용**한다.
우리 자산은 팔이 붙고 마운트 프레임이 추가돼 관성이 다르다. 추출 도구는
`tools/dump_joint_inertia.py` 이고 산출물은 `<asset>_joint_inertia.json` 이다.

⚠**매뉴얼 PDF 와 어긋나는 곳이 있다.** sim 자산을 만드는 것이므로 **USD 를 따른다**
(사용자 확정 2026-09-10): effort USD 7.5 vs 매뉴얼 stall 2.0 · velocity USD π vs 매뉴얼
75 RPM 7.854 · `middle_1` USD ±25° vs 매뉴얼 ±30°. 매뉴얼 정격(연속) 0.4 N·m 경고는 유효하다.
...
### 2-1b. ⚠**좌우 부호 규약이 다르다 — 표에서 추론하지 말 것**

같은 관절이라도 **왼손과 오른손에서 "감싸는 방향"의 부호가 반대인 것이 있다.**
미러는 링크 `origin` 의 `rpy` 에 들어 있고, **관절 `axis` 벡터와 한계표는 좌우가 같아 보인다** —
특히 `_3`/`_4` 는 한계가 ±90° **대칭**이라 표만 보면 좌우 차이가 전혀 드러나지 않는다.

2026-09-10 에 실제로 밟았다: "축이 좌우 동일하니 부호 규약도 같다"고 추론해 **오른손 엄지의
감쌈 부호를 반대로 판정**했고, 그 위에서 "액션 제한이 인벨롭을 막고 있다"는 오진 보고까지 갔다.
FK 로 재보니 정반대였다.

**DG-5F 인벨롭 파지 자세의 부호 규약** (Isaac Sim lula kinematics 실측 + URDF FK 재확인):

| | `_1` | `_2` | `_3` | `_4` |
|---|---|---|---|---|
| **왼손** | 0 유지 | thumb + · index~ring + · **pinky −~0** | **thumb −** · 나머지 + | **thumb −** · 나머지 + |
| **오른손** | 0 유지 | **thumb −** · index~ring + · **pinky 0~+** | **전부 +** | **전부 +** |

**판정 방법은 FK 다.** 인벨롭의 정의대로 **엄지끝 ↔ 검지·중지·약지끝 평균 거리**를 부호별로
재서 작아지는 쪽이 감쌈 방향이다(우손 실측: `thumb_3/_4` +0.9/+0.9 에서 23.6 mm,
−0.9/−0.9 에서 139.7 mm). 한계표·축 벡터·링크 이름으로 추론하지 않는다.

**액션 한계에 이 규약을 박는다.** 이상한 자세가 애초에 도달 불가능해야 한다. 한쪽 방향만
허용하는 clamp 는 **방향을 인코딩**하는 것이므로, 손가락 family 를 한 정규식으로 묶어
일괄 적용하면 안 된다 — 부호가 다른 관절이 그 안에 있으면 그 관절의 필요한 방향이
조용히 사라진다. 소비처는 `hdgp` 프로필의 `hand_action_limit_override` 다.

### 2-2. 확정값 — **벤더 Isaac USD 표를 그대로 쓴다** (2026-09-10 전환)

09.09 에는 매뉴얼 PDF + 자기관통 실측으로 표를 만들었다. 09.10 에 벤더가 자기 Isaac 자산을
배포한다는 것을 확인하고 **그 표로 전환**했다(§0). 소스는 `generate_rl_urdf.py` 의
`_DG5F_M_RANGE_DEG` / `_DG5F_S_RANGE_DEG` 이고, 전 관절 공통으로 effort 7.5 · velocity π 다.

| 자산 | 표 | 비고 |
|---|---|---|
| `openarm_dg5f-m_bi` | `_DG5F_M_RANGE_DEG` | 벤더 `dg5f_right` |
| `openarm_dg5f-m-short_bi` | `_DG5F_M_RANGE_DEG` | 벤더 `dg5f_right_short` 와 **한계 동일**(실측 대조). kp 만 다르다(링크가 가벼워 `I_eq` 가 작다) |
| `openarm_dg5f-s_bi` | `_DG5F_S_RANGE_DEG` | 벤더 `dg5fs_right` — **다른 손이다**. 엄지 회전 ±90°, 굽힘 126~130°, `pinky_2` −37~90° |
| `openarm_rh56f1_bi` · `openarm_gripper_bi` | `None` | Tesollo 자산이 아니다 |

09.09 판단 중 **뒤집힌 것**:

- `_1` × 5 를 `±0.16` 으로 좁혔던 것 → 물리 한계는 **벤더 전폭**으로 되돌렸다. 벌림 억제는
  물리 한계가 아니라 **액션 한계**의 일이다(§2-1b — 인벨롭 규약상 `_1` 은 0 유지).
- `thumb_3/_4` 를 `[0, 1.05]` 로 좁혔던 것 → 벤더 `±90°` 로 되돌렸다. 그 실측은
  `_2` 를 벤더 상한에 둔 채 잰 것이라 조합을 잰 것이었다(§2-1 첫 항목이 경고하는 그 실수).
- ⚠**`pinky_2` 를 외전으로 오인**해 액션에서 `±0.16`(9.2°)으로 잠가 뒀다. 우손에서는
  **0~+90° 가 감싸는 방향**이다. 소지가 감쌈에 10%만 참여할 수 있었다.
...
| 축·한계표로 좌우 부호를 추론 | 한쪽 손의 감쌈 방향을 **반대로** 판정 | 미러는 링크 `origin` 의 `rpy` 에 있다. 부호는 **FK 로 잰다**(§2-1b) |
| 손가락 family 를 한 정규식으로 묶어 한쪽 방향 clamp | 부호가 다른 관절의 **필요한 방향이 사라짐** | 방향을 인코딩하는 clamp 는 **관절별**로 낸다 |
| 이웃 관절을 극단에 둔 채 잰 값을 물리 한계로 승격 | 벤더보다 좁은 가짜 한계가 자산에 박힘 | 한계는 벤더에서 오고, 자세 제약은 **액션 한계**가 진다 |
