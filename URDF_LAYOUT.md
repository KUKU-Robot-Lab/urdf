...
- `assemblies/openarm_modular_dual_tesollo_short.xacro` (DG5F short mount/base hands)
- `assemblies/openarm_modular_dual_tesollo_s.xacro` (DG5F-S hands)
- `assemblies/openarm_left_gripper_bimanual_real.xacro`
- `eef/` (`eef/dg5fs/*.xacro` are generated by `tools/gen_dg5fs_xacro.py` from `vendor/tesollo_model/dg5fs`;
  `eef/dg5f_short/*.xacro` by `tools/gen_dg5f_short_xacro.py` from `vendor/tesollo_model/dg5f` — regenerate, do not edit)
- `config/`
- `launch/`
- `scripts/`
- `tools/`
...
- `generated/source/openarm_tesollo_bi_short.urdf` (DG5F short mount/base both hands)
...
- `generated/rl/openarm_dg5f-m-short_bi_rl.urdf` + `_manifest.yaml` (DG5F with the short
  mount/base: identical finger chain and joint names to the DG5F asset — `control_joint_order`,
  `link_order` and `source_to_canonical_joints` are byte-identical — but `*_hj_palm` sits at
  z=0.022 instead of 0.0698, so the palm is 47.8mm closer to the flange. ★Its source is
  `vendor/tesollo_model/dg5f` (the CAD release, per 2026-09-08 user decision), NOT the
  `dg_description` / `dg5f_ros2` driver copies: those ship the same hand with rounded
  placeholder masses (1.685 kg, inconsistent with their own inertia tensors) and a different
  set of joint limits on 9 joints. This asset therefore carries CAD masses (1.570 kg per hand,
  no `HAND_MASS_TARGET_KG` scaling) and CAD limits — which differ from `openarm_dg5f-m_bi_rl`,
  e.g. `*_hj_thumb_1` upper 0.890 → 1.344 rad. Verify the real hand's range before commanding it.)
...
`openarm_dg5f-m_bi_{right,left}`, `openarm_dg5f-m-short_bi_{right,left}`,
